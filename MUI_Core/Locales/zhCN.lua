local L = _G.LibStub("AceLocale-3.0"):NewLocale("MayronUI", "zhCN");
if not L then
  return
end

L["Character"] = "特点";
L["Bags"] = "包";
L["Friends"] = "朋友们";
L["Guild"] = "公会";
L["Help Menu"] = "帮助菜单";
L["PVP"] = "PVP";
L["Spell Book"] = "拼写书";
L["Talents"] = "人才";
L["Achievements"] = "成就";
L["Glyphs"] = "字形";
L["Calendar"] = "日历";
L["LFD"] = "LFD";
L["LFG"] = "LFG";
L["Enable Max Camera Zoom"] = "启用最大相机缩放";
L["Move AddOn Buttons"] = "移动插件按钮";
L["MOVE_ADDON_BUTTONS_TOOLTIP"] = "如果选中，出现在小地图顶部的插件图标按钮将被移动到小地图的右键菜单中。";
L["Raid"] = "袭击";
L["Encounter Journal"] = "邂逅日记";
L["Collections Journal"] = "收藏杂志";
L["Macros"] = "宏";
L["Skills"] = "技能";
L["World Map"] = "世界地图";
L["Quest Log"] = "任务日志";
L["Show Online Friends"] = "显示在线朋友";
L["Toggle Friends List"] = "切换好友列表";
L["Show Guild Members"] = "显示公会成员";
L["Toggle Guild Pane"] = "切换公会窗格";
L["Quests"] = "任务";
L["Save Position"] = "保存位置";
L["Experience"] = "经验";
L["Reputation"] = "名声";
L["Artifact"] = "神器";
L["Azerite"] = "艾泽里特";
L["PVP Score"] = "PVP 分数";
L["Currency"] = "货币";
L["MUI Layout Button"] = "MUI 布局按钮";
L["Left Click:"] = "左键单击：";
L["Switch Layout"] = "切换布局";
L["Right Click:"] = "右键点击：";
L["Show Layout Config Tool"] = "显示布局配置工具";
L["Edit Box (Message Input Box)"] = "编辑框(消息输入框)";
L["Background Color"] = "背景颜色";
L["Backdrop Inset"] = "背景插入";
L["Chat Frame Options"] = "聊天框选项";
L["Enable Chat Frame"] = "启用聊天框";
L["Options"] = "选项";
L["Button Swapping in Combat"] = "战斗中的按钮交换";
L["Standard Chat Buttons"] = "标准聊天按钮";
L["Left Button"] = "左键";
L["Middle Button"] = "中间按钮";
L["Right Button"] = "右键";
L["Shift"] = "转移";
L["Alt"] = "阿尔特";
L["Set the border size."] = "设置边框大小。";
L["Cannot toggle menu while in combat."] = "在战斗中无法切换菜单。";
L["Cannot switch layouts while in combat."] =
  "在战斗中无法切换布局。";
L["Cannot install while in combat."] = "在战斗中无法安装。";
L["Set the spacing between the background and the border."] =
  "设置背景和边框之间的间距。";
L["Allow the use of modifier keys to swap chat buttons while in combat."] =
  "允许使用修饰键在战斗中交换聊天按钮。";
L["List of slash commands:"] = "斜线命令列表：";
L["Welcome back"] = "欢迎回来";
L["Starter Edition accounts cannot perform this action."] =
  "Starter Edition 帐户无法执行此操作。";
L["Loot Specialization set to: Current Specialization"] =
  "战利品专精设置为：当前专精";
L["Must be level 10 or higher to use Talents."] =
  "必须达到 10 级或更高才能使用天赋。";
L["Requires level 10+ to view the PVP window."] =
  "需要 10 级以上才能查看 PVP 窗口。";
L["Requires being inside a Battle Ground."] = "需要在战场内。";
L["Choose Spec"] = "选择规格";
L["Choose Loot Spec"] = "选择战利品规格";
L[" (current)"] = " （当前的）";
L["Toggle Bags"] = "切换袋";
L["Sort Bags"] = "分拣袋子";
L["Commands"] = "命令";
L["Armor"] = "盔甲";
L["Head"] = "头";
L["Shoulder"] = "肩膀";
L["Chest"] = "胸部";
L["Waist"] = "腰部";
L["Legs"] = "腿";
L["Feet"] = "脚";
L["Wrist"] = "手腕";
L["Hands"] = "手";
L["Main Hand"] = "主手";
L["Secondary Hand"] = "副手";
L["Zone"] = "区";
L["Rank"] = "秩";
L["<none>"] = "<无>";
L["Notes"] = "笔记";
L["Achievement Points"] = "成就点";
L["No Guild"] = "没有公会";
L["No Spec"] = "无规格";
L["Current Money"] = "现款";
L["Start of the day"] = "一天的开始";
L["Today's profit"] = "今天的利润";
L["Money per character"] = "每个角色的钱";
L["Guild Chat"] = "公会聊天";
L["Whispers"] = "窃窃私语";
L[" CastBar not enabled."] = " CastBar 未启用。";
L["Lock"] = "锁";
L["Unlock"] = "开锁";
L["Appearance"] = "外貌";
L["Bar Texture"] = "条形纹理";
L["Border"] = "边界";
L["Border Size"] = "边框尺寸";
L["Frame Inset"] = "框架插入";
L["Colors"] = "颜色";
L["Normal Casting"] = "普通铸造";
L["Not Interruptible"] = "不可中断";
L["Finished Casting"] = "完成铸造";
L["Interrupted"] = "中断";
L["Latency"] = "潜伏";
L["Individual Cast Bar Options"] = "个别铸造栏选项";
L["Enable Bar"] = "启用栏";
L["Show Icon"] = "显示图标";
L["Show Latency Bar"] = "显示延迟条";
L["Anchor to SUF Portrait Bar"] = "锚定到 SUF 肖像栏";
L["Width"] = "宽度";
L["Height"] = "高度";
L["Frame Strata"] = "框架层";
L["Frame Level"] = "帧级";
L["Point"] = "观点";
L["Relative Frame"] = "相对框架";
L["Relative Point"] = "相对点";
L["X-Offset"] = "X 偏移";
L["Y-Offset"] = "Y 偏移";
L["Player"] = "玩家";
L["Target"] = "目标";
L["Focus"] = "重点";
L["Mirror"] = "镜子";
L["Pet"] = "宠物";
L["If enabled the Cast Bar will be fixed to the %s Unit Frame's Portrait Bar (if it exists)."] =
  "如果启用,施法栏将固定在 %s 单位框架的肖像栏上（如果存在）。";
L["The %s Unit Frames's Portrait Bar needs to be enabled to use this feature."] =
  "需要启用 %s 单位框架的肖像栏才能使用此功能。";
L["Only takes effect if the Cast Bar is not anchored to a SUF Portrait Bar."] =
  "仅当施法条未锚定到 SUF 肖像条时才会生效。";
L["Manual positioning only works if the CastBar is not anchored to a SUF Portrait Bar."] =
  "手动定位仅在 CastBar 未锚定到 SUF 纵向栏时才有效。";
L["CTRL + Drag:"] = "CTRL + 拖动：";
L["SHIFT + Drag:"] = "SHIFT + 拖动：";
L["Mouse Wheel:"] = "鼠标滚轮：";
L["ALT + Left Click:"] = "ALT + 左键单击：";
L["Move Minimap"] = "移动小地图";
L["Resize Minimap"] = "调整小地图大小";
L["Ping Minimap"] = "Ping 小地图";
L["Show Menu"] = "显示菜单";
L["Zoom in/out"] = "放大/缩小";
L["Toggle this Tooltip"] = "切换此工具提示";
L["New Event!"] = "新事件！";
L["Tracking Menu"] = "跟踪菜单";
L["Cannot access config menu while in combat."] =
  "在战斗中无法访问配置菜单。";
L["Choose Theme:"] = "选择主题：";
L["Theme"] = "主题";
L["Choose Profile:"] = "选择个人资料：";
L["Confirm"] = "确认";
L["Cancel"] = "取消";
L["Top Left"] = "左上方";
L["Top Right"] = "右上";
L["Bottom Left"] = "左下方";
L["Bottom Right"] = "右下角";
L["Adjust the UI Scale:"] = "调整 UI 比例：";
L["Install"] = "安装";
L["INSTALL"] = "安装";
L["CUSTOM INSTALL"] = "自定义安装";
L["INFORMATION"] = "信息";
L["Warning:"] = "警告：";
L["This will reload the UI!"] = "这将重新加载用户界面！";
L["Setup Menu"] = "设置菜单";
L["VERSION"] = "版本";
L["This will ensure that frames are correctly positioned to match the UI scale during installation."] =
  "这将确保框架在安装过程中正确定位以匹配 UI 比例。";
L["General Options"] = "常规选项";
L["Sort By Time Remaining"] = "按剩余时间排序";
L["Show Tooltips On Mouseover"] = "在鼠标悬停时显示工具提示";
L["Create New Field"] = "创建新字段";
L["TimerBar field '%s' created."] = "已创建 TimerBar 字段 '%s'。";
L["TimerBar field '%s' does not exist."] = "TimerBar 字段 '%s' 不存在。";
L["Remove Field"] = "删除字段";
L["Existing Timer Bar Fields"] = "现有的计时器栏字段";
L["Enable Field"] = "启用字段";
L["<%s Field>"] = "<%s 字段>";
L["Unit to Track"] = "要跟踪的单位";
L["TargetTarget"] = "目标目标";
L["FocusTarget"] = "焦点目标";
L["Up"] = "向上";
L["Down"] = "下";
L["Bar Width"] = "条宽";
L["Bar Height"] = "酒吧高度";
L["Bar Spacing"] = "条间距";
L["Show Icons"] = "显示图标";
L["Show Spark"] = "显示火花";
L["Buff Bar Color"] = "抛光条颜色";
L["Debuff Bar Color"] = "减益条颜色";
L["Manual Positioning"] = "手动定位";
L["Text Options"] = "文本选项";
L["Font Size"] = "字体大小";
L["Font Type"] = "字体类型";
L["The unit who is affected by the spell."] = "受法术影响的单位。";
L["The field's vertical growth direction:"] =
  "该领域的垂直增长方向：";
L["Reload UI"] = "重新加载用户界面";
L["General"] = "一般的";
L["Master Font"] = "大师字体";
L["Display Lua Errors"] = "显示 Lua 错误";
L["Set Theme Color"] = "设置主题颜色";
L["Anchor to Side Bar"] = "锚定到侧边栏";
L["Unit Panels"] = "单元面板";
L["Enable Unit Panels"] = "启用单位面板";
L["Symmetric Unit Panels"] = "对称单元面板";
L["Name Panels"] = "名称面板";
L["Action Bar Panels"] = "操作栏面板";
L["Control"] = "控制";
L["SUF Portrait Gradient"] = "SUF人像渐变";
L["Enable Gradient Effect"] = "启用渐变效果";
L["Gradient Colors"] = "渐变色";
L["Start Color"] = "起始颜色";
L["End Color"] = "结束颜色";
L["Target Class Colored"] = "目标类有色";
L["Row"] = "排";
L["Enabled"] = "启用";
L["Default value is"] = "默认值为";
L["Show Text"] = "显示文本";
L["General Data Text Options"] = "通用数据文本选项";
L["Block in Combat"] = "在战斗中阻止";
L["Auto Hide Menu in Combat"] = "战斗中自动隐藏菜单";
L["Spacing"] = "间距";
L["Menu Width"] = "菜单宽度";
L["Max Menu Height"] = "最大菜单高度";
L["Bar Strata"] = "酒吧地层";
L["Bar Level"] = "酒吧级别";
L["Data Text Modules"] = "数据文本模块";
L["Durability"] = "耐用";
L["Performance"] = "表现";
L["Memory"] = "记忆";
L["Money"] = "钱";
L["Disabled"] = "残疾";
L["Module Options"] = "模块选项";
L["Show FPS"] = "显示帧率";
L["Show Server Latency (ms)"] = "显示服务器延迟（毫秒）";
L["Show Home Latency (ms)"] = "显示家庭延迟（毫秒）";
L["Show Total Slots"] = "显示总插槽";
L["Show Used Slots"] = "显示已使用的插槽";
L["Show Free Slots"] = "显示免费老虎机";
L["Show Self"] = "《展现自我》";
L["Show Tooltips"] = "显示工具提示";
L["Never"] = "绝不";
L["Always"] = "总是";
L["On Mouse-over"] = "鼠标悬停";
L["Bar"] = "酒吧";
L["Uncheck to prevent MUI from changing the game font."] =
  "取消选中以防止 MUI 更改游戏字体。";
L["The UI requires reloading to apply changes."] =
  "用户界面需要重新加载才能应用更改。";
L["Some changes require a client restart to take effect."] =
  "某些更改需要重新启动客户端才能生效。";
L["Previously called 'Classic Mode'."] = "以前称为'经典模式'。";
L["Allow MUI to Control Unit Frames"] = "允许 MUI 控制单元框架";
L["What color the gradient should start as."] =
  "渐变应该以什么颜色开始。";
L["What color the gradient should change into."] =
  "渐变应该变成什么颜色。";
L["Show your character in the guild list."] =
  "在公会列表中展示你的角色。";
L["Adjust the width of the unit frame background panels."] =
  "调整单元框背景面板的宽度。";
L["Adjust the width of the unit name background panels."] =
  "调整单位名称背景面板的宽度。";
L["Adjust the height of the unit name background panels."] =
  "调整单位名称背景面板的高度。";
L["Adjust the width of the Objective Tracker."] =
  "调整目标跟踪器的宽度。";
L["Adjust the height of the Objective Tracker."] =
  "调整目标跟踪器的高度。";
L["Move the unit name panels further in or out."] =
  "将单位名称面板进一步移入或移出。";
L["Set the font size of unit names."] = "设置单位名称的字体大小。";
L["The speed of the Expand and Retract transitions."] =
  "Expand 和 Retract 转换的速度。";
L["The higher the value, the quicker the speed."] =
  "数值越高,速度越快。";
L["The height of the gradient effect."] = "渐变效果的高度。";
L["Adjust the spacing between data text buttons."] =
  "调整数据文本按钮之间的间距。";
L["The frame strata of the entire DataText bar."] =
  "整个 DataText 栏的框架层。";
L["Disable this to stop MUI from controlling the Objective Tracker."] =
  "禁用此功能可阻止 MUI 控制目标跟踪器。";
L["Adjust the horizontal positioning of the Objective Tracker."] =
  "调整目标跟踪器的水平位置。";
L["Adjust the vertical positioning of the Objective Tracker."] =
  "调整目标跟踪器的垂直定位。";
L["The font size of text that appears on data text buttons."] =
  "出现在数据文本按钮上的文本字体大小。";
L["Show guild info tooltips when the cursor is over guild members in the guild list."] =
  "当光标位于公会列表中的公会成员上时,显示公会信息工具提示。";
L["The frame level of the entire DataText bar based on its frame strata value."] =
  "基于其帧层值的整个 DataText 条的帧级别。";
L["If unchecked, the entire DataText module will be disabled and all"] =
  "如果取消选中,整个 DataText 模块将被禁用和所有";
L["DataText buttons, as well as the background bar, will not be displayed."] =
  "DataText 按钮以及背景栏将不会显示。";
L["Prevents you from using data text modules while in combat."] =
  "阻止你在战斗中使用数据文本模块。";
L["If the SUF Player or Target portrait bars are enabled, a class colored gradient will overlay it."] =
  "如果启用了 SUF Player 或 Target 纵向条,类颜色渐变将覆盖它。";
L["Show Overview"] = "显示概述";
L["Show Reset Options"] = "显示重置选项";
L["Reset All Characters"] = "重置所有字符";
L["Are you sure you want to reset the money data for all of your characters?"] =
  "您确定要重置所有角色的金钱数据吗？";
L["All money data has been reset."] = "所有货币数据已重置。";
L["Are you sure you want to reset the money data for %s?"] =
  "您确定要重置 %s 的金钱数据吗？";
L["Money data for %s has been reset."] = "%s 的金钱数据已被重置。";
L["Reset Options"] = "重置选项";
L["Change Status"] = "更改状态";
L["Remove from Whitelist"] = "从白名单中删除";
L["Add to Blacklist"] = "加入黑名单";
L["Are you sure you want to do this?"] = "你确定要这么做吗？";
L["%s has been removed from the whitelist."] = "%s 已从白名单中删除。";
L["%s has been added to the blacklist."] = "%s 已加入黑名单。";
L["Filters"] = "过滤器";
L["Only show buffs applied by me"] = "只显示我应用的增益";
L["Only show debuffs applied by me"] = "只显示我施加的减益效果";
L["Enable Whitelist"] = "启用白名单";
L["Configure Whitelist"] = "配置白名单";
L["Enable Blacklist"] = "启用黑名单";
L["Configure Blacklist"] = "配置黑名单";
L["Removing %s from the whitelist will hide this timer bar if the whitelist is enabled."] =
  "如果启用了白名单,从白名单中删除 %s 将隐藏此计时器栏。";
L["Adding %s to the blacklist will hide this timer bar if the blacklist is enabled."] =
  "如果启用了黑名单,将 %s 添加到黑名单将隐藏此计时器栏。";
L["Cast Bars"] = "铸棒";
L["(CTRL+C to Copy, CTRL+V to Paste)"] = "(CTRL+C 复制,CTRL+V 粘贴)";
L["Copy Chat Text"] = "复制聊天文本";
L["Data Text Bar"] = "数据文本栏";
L["Setup"] = "设置";
L["Timer Bars"] = "计时器栏";
L["MUI Profile Manager"] = "MUI 配置文件管理器";
L["Current profile"] = "当前个人资料";
L["Reset Profile"] = "重置配置文件";
L["Delete Profile"] = "删除个人资料";
L["Copy From"] = "复制自";
L["Select profile"] = "选择个人资料";
L["Choose Profile"] = "选择个人资料";
L["Choose the currently active profile."] =
  "选择当前活动的配置文件。";
L["New Profile"] = "新的配置文件";
L["Create a new profile"] = "创建一个新的配置文件";
L["Default Profile Behaviour"] = "默认配置文件行为";
L["Name of New Layout"] = "新布局名称";
L["Layout"] = "布局";
L["Rename Layout"] = "重命名布局";
L["Are you sure you want to delete Layout '%s'?"] =
  "您确定要删除布局 '%s' 吗？";
L["MUI Layout Tool"] = "MUI 布局工具";
L["Layouts"] = "布局";
L["Create New Layout"] = "创建新布局";
L["Delete Layout"] = "删除布局";
L["Chat Buttons with Modifier Key %d"] = "带有修饰键 %d 的聊天按钮";
L["Please install the UI and try again."] = "请安装用户界面并重试。";
L["Chat Frames"] = "聊天框";
L["Mini-Map Options"] = "小地图选项";
L["Mini-Map"] = "小地图";
L["Adjust the size of the minimap."] = "调整小地图的大小。";
L["Adjust the scale of the minimap."] = "调整小地图的比例。";
L["Scale"] = "规模";
L["Size"] = "尺寸";
L["Okay"] = "好的";
L["Profile %s has been copied into current profile %s."] =
  "配置文件 %s 已复制到当前配置文件 %s。";
L["Reset currently active profile back to default settings."] =
  "将当前活动的配置文件重置为默认设置。";
L["Are you sure you want to reset profile '%s' back to default settings?"] =
  "您确定要将配置文件 '%s' 重置为默认设置吗？";
L["Delete currently active profile (cannot delete the 'Default' profile)."] =
  "删除当前活动的配置文件（不能删除'默认'配置文件）。";
L["Copy all settings from one profile to the active profile."] =
  "将所有设置从一个配置文件复制到活动配置文件。";
L["Are you sure you want to override all profile settings in '%s' for those in profile '%s'?"] =
  "您确定要为配置文件 '%s' 中的配置文件覆盖 '%s' 中的所有配置文件设置吗？";
L["Profile Per Character"] = "每个角色的个人资料";
L["If enabled, new characters will be assigned a unique character profile instead of the Default profile."] =
  "如果启用,新角色将被分配一个唯一的角色配置文件,而不是默认配置文件。";
L["Anchor the Objective Tracker to the action bar container on the right side of the screen."] =
  "将目标跟踪器锚定到屏幕右侧的操作栏容器。";
L["If true, the objective tracker will collapse when entering an instance."] =
  "如果为 true,目标跟踪器将在进入实例时崩溃。";
L["Also enable the %s"] = "同时启用 %s";
L["Current"] = "当前的";
L["Profiles"] = "个人资料";
L["AFK Display"] = "AFK 显示";
L["Movable Frames"] = "可移动框架";
L["Objective Tracker"] = "目标跟踪器";
L["(type '/mui' to list all slash commands)"] =
  "(输入'/mui'列出所有斜线命令)";
L["Version"] = "版本";
L["Resource Bars"] = "资源栏";
L["Allows you to move Blizzard Frames outside of combat only."] =
  "只允许你在战斗之外移动暴雪框架。";
L["Reset Blizzard frames back to their original position."] =
  "将暴雪框架重置回原来的位置。";
L["Enable/disable the AFK Display"] = "启用/禁用 AFK 显示";
L["Show AFK Display"] = "显示 AFK 显示";
L["Adjust the width of the main container."] = "调整主容器的宽度。";
L["Adjust the height of the unit frame background panels."] =
  "调整单位框架背景面板的高度。";
L["Side Action Bars"] = "侧面动作条";
L["Collapse in Instance"] = "在实例中折叠";
L["CREDITS"] = "信用";
L["Buffs"] = "爱好者";
L["Debuffs"] = "减益";
L["Show Pulse Effect"] = "显示脉冲效果";
L["Icon Options"] = "图标选项";
L["Column Spacing"] = "列间距";
L["Row Spacing"] = "行间距";
L["Icons per Row"] = "每行图标";
L["Bar Columns"] = "酒吧专栏";
L["Growth Direction"] = "《成长方向》";
L["Status Bar Options"] = "状态栏选项";
L["Time Remaining"] = "剩余时间";
L["Spell Count"] = "法术计数";
L["Spell Name"] = "法术名称";
L["Count"] = "数数";
L["Aura Name"] = "光环名称";
L["Border Type"] = "边框类型";
L["Magic Debuff"] = "魔法减益";
L["Disease Debuff"] = "疾病减益";
L["Poison Debuff"] = "毒药减益";
L["Curse Debuff"] = "诅咒减益";
L["Icons"] = "图标";
L["Status Bars"] = "状态栏";
L["Can Steal or Purge"] = "可以窃取或清除";
L["If an aura can be stolen or purged, show a different color."] =
  "如果光环可以被窃取或清除,则显示不同的颜色。";
L["Show Borders"] = "显示边界";
L["Enter an aura name to add to the whitelist:"] =
  "输入要添加到白名单的光环名称：";
L["Enter an aura name to add to the blacklist:"] =
  "输入要添加到黑名单的光环名称：";
L["Inventory"] = "存货";
L["Changes to these settings will take effect after 0-3 seconds."] =
  "对这些设置的更改将在 0-3 秒后生效。";
L["Adjust the height of the datatext bar."] =
  "调整数据文本栏的高度。";
L["Set the spacing between the status bar and the background."] =
  "设置状态栏和背景之间的间距。";
L["Select which chat frame the chat icons should be anchored to."] =
  "选择聊天图标应该锚定到哪个聊天框架。";
L["The height of the edit box."] = "编辑框的高度。";
L["Set the vertical positioning of the edit box."] =
  "设置编辑框的垂直定位。";
L["This is the background bar that goes behind the tabs."] =
  "这是标签后面的背景栏。";
L["Show Tab Bar"] = "显示标签栏";
L["Tab Bar"] = "标签栏";
L["Window"] = "窗户";
L["Layout enabled!"] = "布局已启用！";
L["Show Chat Menu"] = "显示聊天菜单";
L["Covenant Sanctum"] = "《圣约圣殿》";
L["You must be a member of a covenant to view this."] =
  "你必须是盟约的成员才能看到这个。";
L["Unmuted"] = "未静音";
L["Toggle Questlog"] = "切换任务日志";
L["Step %d of"] = "第 %d 步";
L["Step"] = "步";
L["Found a bug? Use this form to submit an issue to the official MayronUI GitHub page."] =
  "发现错误？使用此表单向官方 MayronUI GitHub 页面提交问题。";
L["Almost done! We just need a bit more information..."] =
  "差不多完成了！我们只是需要更多信息……";
L["Click below to generate your report. Once generated, copy it into a new issue and submit it on GitHub using the link below:"] =
  "点击下方生成您的报告。生成后,将其复制到一个新问题中并使用以下链接在 GitHub 上提交：";
L["characters"] = "characters";
L["minimum"] = "最低限度";
L["Please describe the bug in detail:"] = "请详细描述错误：";
L["How can we replicate the bug?"] = "我们如何复制错误？";
L["Report Issue"] = "报告问题";
L["Report MayronUI Bug"] = "С报告 MayronUI 错误";
L["Open this webpage in your browser"] = "在浏览器中打开此网页";
L["Generate Report"] = "生成报告";
L["Copy Report"] = "复制报告";
L["Chat Frame with Icons"] = "带图标的聊天框";
L["Vertical Side Icons"] = "垂直侧面图标";
L["Only 1 active chat frame can show the chat icons on the sidebar (see icons listed below)."] =
  "只有 1 个活动的聊天框可以在侧边栏上显示聊天图标（请参阅下面列出的图标）。";
L["Horizontal Top Buttons"] = "水平顶部按钮";
L["This option will affect all active chat frames. To configure each individual button per chat frame, see the chat frame sub-menus below."] =
  "此选项将影响所有活动的聊天框。要为每个聊天框配置每个单独的按钮,请参阅下面的聊天框子菜单。";
L["Chat Channels"] = "聊天频道";
L["Professions"] = "《职业》";
L["AddOn Shortcuts"] = "附加快捷方式";
L["Copy Chat"] = "复制聊天";
L["Emotes"] = "表情";
L["Online Status"] = "在线状态";
L["None"] = "没有任何";
L["Deafen"] = "耳聋";
L["Mute"] = "沉默的";
L["Unit Tooltip Options"] = "单位工具提示选项";
L["These options only affect tooltips that appear when you mouse over a unit, such as an NPC or player."] =
  "这些选项只影响当你将鼠标悬停在一个单位上时出现的工具提示,比如 NPC 或玩家。";
L["If checked, the target of the unit (NPC or player) displayed in the tooltip will be shown."] =
  "如果选中,将显示工具提示中显示的单位（NPC 或玩家）的目标。";
L["If checked and the player is in a guild, the guild rank of the player displayed in the tooltip will be shown."] =
  "如果勾选并且玩家在公会中,将在提示中显示玩家的公会等级。";
L["If checked and the player is level 10 or higher, the item level of the player displayed in the tooltip will be shown."] =
  "如果选中且玩家等级为10级或更高,将显示提示中显示的玩家物品等级。";
L["The player must be close enough to be inspected for this information to load."] =
  "玩家必须足够近才能被检查以加载此信息。";
L["If checked and the player is level 10 or higher and has chosen a class specialization, the specialization of the player displayed in the tooltip will be shown."] =
  "如果选中并且玩家等级为 10 或更高并且选择了职业专精,则会显示工具提示中显示的玩家专精。";
L["Show Specialization"] = "显示专业化";
L["Show Item Level"] = "显示物品等级";
L["Show Guild Rank"] = "显示公会等级";
L["Show Realm Name"] = "显示领域名称";
L["Show Target"] = "显示目标";
L["Appearance Options"] = "外观选项";
L["These options allow you to customize the appearance of the tooltips."] =
  "这些选项允许您自定义工具提示的外观。";
L["Font Flag"] = "字体标志";
L["Standard Font Size"] = "标准字体大小";
L["Header Font Size"] = "标题字体大小";
L["Affects the overall size of the tooltips."] =
  "影响工具提示的整体大小。";
L["Reskin using the MUI texture or a custom backdrop"] =
  "使用 MUI 纹理或自定义背景重新换肤";
L["Texture"] = "质地";
L["Customize which addOn/s should change to which profile/s for each layout, as well as manage your existing layouts or create new ones."] =
  "为每个布局自定义哪些插件应该更改为哪些配置文件,以及管理现有布局或创建新布局。";
L["Custom Backdrop"] = "自定义背景";
L["Custom Color"] = "自定义颜色";
L["Custom Backdrop Options"] = "自定义背景选项";
L["Background Texture"] = "背景纹理";
L["Border Insets"] = "边框插入";
L["Left"] = "剩下";
L["Right"] = "正确的";
L["Center"] = "中心";
L["Top"] = "最佳";
L["Bottom"] = "底部";
L["The bottom-[point] corner of the tooltip, where [point] is either 'Left' or 'Right', will be anchored to the position of the mouse cursor."] =
  "工具提示的底部-[点] 角,其中 [点] 是'左'或'右',将锚定到鼠标光标的位置。";
L["Health Bar"] = "健康酒吧";
L["Outline"] = "大纲";
L["Thick Outline"] = "粗大纲";
L["Monochrome"] = "单色";
L["Text Format"] = "文本格式";
L["Set the text format of the value that appears on the status bar. Set this to 'None' to hide the text."] =
  "设置出现在状态栏上的值的文本格式。将其设置为'无'以隐藏文本。";
L["Current/Maximum"] = "当前/最大";
L["Percentage"] = "百分比";
L["Power Bar"] = "能量棒";
L["If checked, unit tooltips will show the unit's power bar."] =
  "如果选中,单位工具提示将显示单位的功率条。";
L["Unit Aura Options"] = "单位光环选项";
L["If checked, unit tooltips will show the unit's buffs."] =
  "如果选中,单位工具提示将显示单位的增益效果。";
L["Above"] = "以上";
L["Below"] = "以下";
L["Position"] = "位置";
L["Set whether you want the unit's buffs to appear above or below the tooltip."] =
  "设置你是否希望单位的增益出现在工具提示的上方或下方。";
L["Left to Right"] = "左到右";
L["Right to Left"] = "右到左";
L["If checked, unit tooltips will show the unit's debuffs."] =
  "如果选中,单位工具提示将显示单位的减益效果。";
L["Set border color by debuff type"] = "按减益类型设置边框颜色";
L["If enabled, the border color of debuffs will be based on the type of debuff (e.g., poisons will appear with a green border color)."] =
  "如果启用,debuff 的边框颜色将基于 debuff 的类型（例如,毒药会以绿色边框颜色出现）。";
L["Set whether you want the unit's debuffs to appear above or below the tooltip."] =
  "设置你想让单位的减益效果出现在工具提示的上方还是下方。";
L["Buff and Debuff Ordering"] = "增益和减益排序";
L["Debuffs Above Buffs"] = "减益高于增益";
L["Buffs Above Debuffs"] = "增益高于减益";
L["Non-Player Alpha"] = "非玩家阿尔法";
L["Sets the alpha of timer bars for auras not produced by you (i.e., other player or NPC buffs and debuffs)."] =
  "为不是由你产生的光环设置计时器条的 alpha (即其他玩家或 NPC 的增益和减益)。";
L["Show Professions"] = "显示职业";
L["Show AddOn Shortcuts"] = "显示插件快捷方式";
L["Profile Manager"] = "档案管理器";
L["Config Menu"] = "配置菜单";
L["Show Profiles"] = "显示个人资料";
L["Report"] = "报告";
L["Toggle Alignment Grid"] = "切换对齐网格";
L["Guild Bank"] = "公会银行";
L["Bank"] = "银行";
L["Void Storage"] = "空仓";
L["You have no professions."] = "你没有职业。";
L["remaining"] = "其余的";
L["Fixed Color"] = "固定颜色";
L["Main Container"] = "主容器";
L["Talking Head Frame"] = "会说话的头架";
L["This is the animated character portrait frame that shows when an NPC is talking to you."] =
  "这是动画人物头像框,显示 NPC 何时与您交谈。";
L["Top of Screen"] = "屏幕顶部";
L["Bottom of Screen"] = "屏幕底部";
L["Pulse While Resting"] = "休息时脉搏";
L["If enabled, the unit panels will fade in and out while resting."] =
  "如果启用,单元面板将在休息时淡入淡出。";
L["Target Class Color Gradient"] = "目标类颜色渐变";
L["If enabled, the unit panel color will transition to the target's class color using a horizontal gradient effect."] =
  "如果启用,单元面板颜色将使用水平渐变效果过渡到目标的类颜色。";
L["Set Pulse Strength"] = "设置脉冲强度";
L["Set Alpha"] = "设置阿尔法";
L["If checked, the reputation bar will use a fixed color instead of dynamically changing based on your reputation with the selected faction."] =
  "如果选中,声望栏将使用固定颜色,而不是根据您在所选派系中的声望动态变化。";
L["Use Fixed Color"] = "使用固定颜色";
L["Reputation Colors"] = "声誉颜色";
L["Show"] = "表演";
L["Hide"] = "隐藏";
L["Enable Test Mode"] = "启用测试模式";
L["Disable Test Mode"] = "禁用测试模式";
L["Icon Position"] = "图标位置";
L["If checked, this module will be enabled."] =
  "如果选中，将启用此模块。";
L["Test mode allows you to easily customize the looks and positioning of widgets by forcing all widgets to be shown."] =
  "测试模式允许您通过强制显示所有小部件来轻松自定义小部件的外观和位置。";
L["Mini-Map Widgets"] = "小地图小工具";
L["Clock"] = "钟";
L["Dungeon Difficulty"] = "地牢难度";
L["Looking For Group Icon"] = "寻找组图标";
L["New Mail Icon"] = "新邮件图标";
L["Missions Icon"] = "任务图标";
L["This button opens the most relevant missions menu for your character. The menu will either show missions for your Covenant Sanctum, Class Order Hall, or your Garrison."] =
  "此按钮为您的角色打开最相关的任务菜单。菜单将显示您的圣约圣殿、职业秩序大厅或您的要塞的任务。";
L["Tracking Icon"] = "跟踪图标";
L["When hidden, you can still access tracking options from the Minimap right-click menu."] =
  "隐藏后，您仍然可以从小地图右键单击菜单访问跟踪选项。";
L["Zone Name"] = "区域名称";
L["Apply Scaling"] = "应用缩放";
L["Show MUI Key Bindings"] = "显示 MUI 键绑定";
L["Tutorial: Step 2"] = "教程：第 2 步";
L["Tutorial: Step 1"] = "教程：第 1 步";
L["Open Config Menu"] = "打开配置菜单";
L["Config"] = "配置";
L["Unit Frame Panels"] = "单元框架面板";
L["Tooltips"] = "工具提示";
L["Show Food and Drink"] = "显示食物和饮料";
L["If checked, the food and drink buff will be displayed as a castbar."] =
  "如果选中,食物和饮料buff将显示为一个castbar。";
L["Blend Mode"] = "混合模式";
L["Changing the blend mode will affect how alpha channels blend with the background."] =
  "更改混合模式将影响 Alpha 通道与背景混合的方式。";
L["Background"] = "背景";
L["Button"] = "按钮";
L["Combat Timer"] = "战斗计时器";
L["Volume Options"] = "音量选项";
L["Specialization"] = "专业化";
L["Show Auras With Unknown Time Remaining"] =
  "显示剩余时间未知的光环";
L["Show Buffs"] = "显示爱好者";
L["Show Debuffs"] = "显示减益";
L["Refresh Chat Text"] = "刷新聊天文本";
L["Back"] = "后退";
L["Next"] = "下一个";
L["Set short, custom aliases for chat channel names."] =
  [[为聊天频道名称设置简短的自定义别名。]]
L["The main container holds the unit frame panels, action bar panels, data-text bar, and all resource bars at the bottom of the screen."] =
  "主容器包含单元框架面板、操作栏面板、数据文本栏和屏幕底部的所有资源栏。";
L["Add Text Highlighting"] = "添加文本突出显示";
L["Channel Name Aliases"] = "频道名称别名";
L["Text Highlighting"] = "文本突出显示";
L["Alias Brightness"] = "别名亮度";
L["Auction House Open"] = "拍卖行开放";
L["Auction House Close"] = "拍卖行关闭";
L["Alarm Clock Warning 1"] = "闹钟警告1";
L["Alarm Clock Warning 2"] = "闹钟警告2";
L["Alarm Clock Warning 3"] = "闹钟警告3";
L["Enter Queue"] = "进入队列";
L["Jewel Crafting Socket"] = "珠宝制作插座";
L["Loot Money"] = "抢钱";
L["Map Ping"] = "地图平";
L["Queue Ready"] = "队列准备就绪";
L["Raid Warning"] = "突袭警告";
L["Raid Boss Warning"] = "突袭boss警告";
L["Ready Check"] = "准备检查";
L["Repair Item"] = "维修项目";
L["Whisper Received"] = "耳语收到";
L["Server Channels"] = "服务器通道";
L["Text to Highlight (case insensitive):"] =
  "要突出显示的文本（不区分大小写）：";
L["Enter text to highlight:"] = "输入要突出显示的文本：";
L["Show in Upper Case"] = "以大写显示";
L["Set Color"] = "设置颜色";
L["Edit Text"] = "编辑文本";
L["Play a sound effect when any of the selected text appears in chat."] =
  "当任何选定的文本出现在聊天中时播放声音效果。";
L["Play Sound"] = "播放声音";
L["Set Timestamp Color"] = "设置时间戳颜色";
L["Use Fixed Timestamp Color"] = "使用固定时间戳颜色";
L["Timestamps"] = "时间戳";
L["Enable Custom Aliases"] = "启用自定义别名";
L["Dangerous Actions!"] = "危险动作！";
L["Cannot delete the Default profile."] = "无法删除默认配置文件。";
L["Are you sure you want to delete profile '%s'?"] =
  "您确定要删除配置文件 '%s' 吗？";
L["DELETE"] = "删除";
L["Please type '%s' to confirm:"] = "请输入'%s' 来确认：";
L["Enter a new unique profile name:"] =
  "输入一个新的唯一配置文件名称：";
L["Current Profile:"] = "当前档案：";
L["Show the MUI Config Menu"] = "显示 MUI 配置菜单";
L["Show the MUI Installer"] = "显示 MUI 安装程序";
L["List All Profiles"] = "列出所有配置文件";
L["Show the MUI Profile Manager"] = "显示 MUI 配置文件管理器";
L["Set Profile"] = "设置配置文件";
L["Show Currently Active Profile"] = "显示当前活动的个人资料";
L["Show the Version of MUI"] = "显示 MUI 版本";
L["Show the MUI Layout Tool"] = "显示 MUI 布局工具";
L["Some settings will not be changed until the UI has been reloaded."] =
  "在重新加载 UI 之前，某些设置不会更改。";
L["Would you like to reload the UI now?"] =
  "您现在要重新加载用户界面吗？";
L["No"] = "不";
L["Profile changed to %s."] = "配置文件已更改为 %s。";
L["Profile %s has been reset."] = "配置文件 %s 已被重置。";

---------------------------------
--- LARGE TEXT:
---------------------------------
L["PRESS_HOLD_TOGGLE_BUTTONS"] =
[[脱离战斗时按住 %s 可显示切换按钮。

单击这些将显示或隐藏其他操作栏行。]]

L["CHANGE_KEYBINDINGS"] =
[[您可以在 MUI 配置菜单 (%s) 中更改此组合键。

有 3 个按键绑定，可以在 1 到 3 行之间快速切换，
在暴雪键绑定菜单中找到：]]

L["THANK_YOU_FOR_INSTALLING"] = [[感谢您安装 MayronUI！

您可以使用配置菜单完全自定义 UI：]]

L["SHOW_AURAS_WITH_UNKNOWN_TIME_TOOLTIP"] =
  [[如果启用，剩余时间未知的光环，以及永远不会的光环
除非取消，否则过期，将显示在计时器栏字段上，并将
永不枯竭。 如果光环被移除，该条仍然会消失。

经典中，部分光环剩余时间（视情况而定）
玩家可能不知道，除非启用此功能，否则将被隐藏。]]

L["TT_MUI_CONTROL_SUF"] = [[如果启用，MUI 将重新定位阴影单元
安装在 MUI 单元面板顶部的框架。

它也会在以下情况下自动移动单位框架
展开和缩回 MUI 操作栏面板。]];

L["TT_MUI_USE_TARGET_CLASS_COLOR"] =
  [[如果选中，目标人像渐变将使用目标的类
颜色而不是使用“起始颜色”RGB 值。 它会
仍然使用 Alpha 和 'End Color' RGB 值。]];

L["MUI_Setup_InfoTab"] = [[不和谐社区：
%s

访问官方 MayronUI 网站了解更多信息：
%s

官方 GitHub 仓库：
%s

成为赞助人并在社区内获得独家福利：
%s

访问 Mayron 的官方 YouTube 频道：
%s]]

L["MUI_Setup_CreditsTab"] =
  [[特别感谢以下 MayronUI 社区成员对项目的贡献（请参阅信息标签以获取加入我们 Discord 社区的链接）：

|cff00ccff> 赞助人|r
%s

|cff00ccff> 开发和错误修复|r
%s

|cff00ccff> 翻译支持|r
%s

|cff00ccff> 社区支持团队|r
%s

当然，感谢此 UI 包中包含的非 MayronUI 插件的作者。]]

L["AURAS_ORDERING_ON_TOOLTIP"] =
  [[以下设置控制工具提示上的光环顺序
当 buffs 和 debuffs 放在一起时
（在工具提示上方或下方）并且都已启用。]]

L["MANAGE_TEXT_HIGHLIGHTING"] = [[管理文本以突出显示和可选
当他们出现在聊天中时播放声音效果。]]

L["NO_HIGHLIGHT_TEXT_ADDED"] = [[您还没有添加任何文字！
按下面的“编辑文本”按钮添加要突出显示的文本：]];

L["MANAGE_PROFILES_HERE"] = [[您可以在此处管理角色档案。

默认情况下,每个角色都有自己独特的档案。]]

L["UNIQUE_CHARACTER_PROFILE"] =
  [[默认情况下,每个新角色将自动分配一个唯一的角色配置文件,而不是单个默认配置文件。

配置文件仅在新角色上安装 UI 后自动分配。]]

L["Reset Chat Settings"] = "重置聊天设置";
L["RESET_CHAT_SETTINGS_TOOLTIP"] = "取消选中此项将保留您的聊天选项卡以及与每个聊天窗口关联的暴雪聊天设置。";

L["MayronUI AddOn Presets"] = "MayronUI 插件预设";
L["No Supported AddOns Loaded"] = "没有加载支持的插件";
L["The following selected addons will have their settings reset to the MayronUI preset settings:"] = "以下选定插件的设置将重置为 MayronUI 预设设置：";
L["There is a newer MayronUI preset available for %s."] = "%s 有更新的 MayronUI 预设可用。";
L["Would you like to install it?"] = "您想安装它吗？";
L["Warning! This will wipe all customizations you have made to %s."] = "警告！这将清除您对 %s 所做的所有自定义设置。";
L["Install Preset"] = "安装预设";
L["Skip this Version"] = "跳过这个版本";
L["You can always install presets at anytime from the custom install tab located on the MayronUI installer."] = "您始终可以随时从 MayronUI 安装程序上的自定义安装选项卡安装预设。";
L["Hint: Type %s to access the installer."] = "提示：键入 %s 以访问安装程序。";
L["Set Arrow Button Visibility"] = "设置箭头按钮可见性";
L["Hide Arrow Buttons In Combat"] = "在战斗中隐藏箭头按钮";
L["Set Width Mode"] = "设置宽度模式";
L["If set to dynamic, MayronUI will calculate the optimal width for the selected Bartender4 action bars to fit inside the panel."] = "如果设置为动态，MayronUI 将计算所选 Bartender4 操作栏的最佳宽度以适应面板。";
L["Set Panel Padding"] = "设置面板填充";
L["Set Height Mode"] = "设置高度模式";
L["If set to dynamic, MayronUI will calculate the optimal height for the selected Bartender4 action bars to fit inside the panel."] = "如果设置为动态，MayronUI 将计算所选 Bartender4 操作栏的最佳高度以适应面板。";
L["Set the modifier key/s that should be pressed to show the arrow buttons."] = "设置应该按下以显示箭头按钮的修改键。";
L["Set Animation Speed"] = "设置动画速度";
L["These settings control the MayronUI artwork behind the Bartender4 action bars."] = "这些设置控制 Bartender4 操作栏后面的 MayronUI 图稿。";
L["Background Panel Settings"] = "背景面板设置";
L["Bottom Panel"] = "底部面板";
L["Action Bars"] = "动作条";
L["Popular Settings"] = "热门设置";
L["Global Settings"] = "全局设置";
L["These settings are applied account-wide"] = "这些设置在整个帐户范围内应用";
L["Override Master Font"] = "覆盖主字体";
L["Override Combat Font"] = "覆盖战斗字体";
L["Combat Font"] = "战斗字体";
L["This font is used to display the damage and healing combat numbers."] = "此字体用于显示伤害和治疗战斗数字。";
L["Blizzard Frames"] = "暴雪框架";
L["Clamped to Screen"] = "夹在屏幕上";
L["If checked, Blizzard frames cannot be dragged outside of the screen."] = "如果选中，则无法将暴雪帧拖到屏幕外。";
L["Reset Positions"] = "重置职位";
L["Blizzard frame positions have been reset."] = "暴雪框架位置已重置。";
L["Set the alpha change while pulsing/flashing."] = "在脉冲/闪烁时设置 alpha 变化。";
L["Dynamic"] = "动态的";
L["Manual"] = "手动的";
L["Manual Height Mode Settings"] = "手动高度模式设置";
L["Set Row 1 Height"] = "设置第 1 行高度";
L["Set Row 2 Height"] = "设置第 2 行高度";
L["Set Row 3 Height"] = "设置第 3 行高度";
L["Side Panel"] = "侧面板";
L["Set Y-Offset"] = "设置 Y 偏移";
L["Manual Side Panel Widths"] = "手动侧面板宽度";
L["Set Column 1 Width"] = "设置第 1 列宽度";
L["Set Column 2 Width"] = "设置第 2 列宽度";
L["Bartender4 Override Settings"] = "Bartender4 覆盖设置";
L["These settings control what MayronUI is allowed to do with the Bartender4 action bars. By default, MayronUI:"] = "这些设置控制允许 MayronUI 使用 Bartender4 操作栏执行的操作。默认情况下，MayronUI：";
L["Fades action bars in and out when you press the provided arrow buttons."] = "当您按下提供的箭头按钮时，操作栏会淡入淡出。";
L["Maintains the visibility of action bars between sessions of gameplay."] = "保持游戏会话之间操作栏的可见性。";
L["Sets the scale and padding of action bar buttons to best fit inside the background panels."] = "设置操作栏按钮的比例和填充以最适合背景面板。";
L["Sets and updates the position the action bars so they remain in place ontop of the background panels."] = "设置和更新操作栏的位置，以便它们保留在背景面板的顶部。";
L["Bottom Bartender4 Action Bars"] = "Bottom Bartender4 动作条";
L["If enabled, MayronUI will move the selected Bartender4 action bars into the correct position for you."] = "如果启用，MayronUI 会将选定的 Bartender4 操作栏移动到正确的位置。";
L["Control Bar Positioning"] = "控制栏定位";
L["Override Bar Padding"] = "覆盖栏填充";
L["If enabled, MayronUI will set the padding of the selected Bartender4 action bar to best fit the background panel."] = "如果启用，MayronUI 将设置所选 Bartender4 操作栏的填充以最适合背景面板。";
L["Override Bar Scale"] = "覆盖条形刻度";
L["If enabled, MayronUI will set the scale of the selected Bartender4 action bar to best fit the background panel."] = "如果启用，MayronUI 将设置所选 Bartender4 操作栏的比例以最适合背景面板。";
L["Set Row Spacing"] = "设置行间距";
L["Set Bar Padding"] = "设置条形填充";
L["Set Bar Scale"] = "设置条刻度";
L["The bottom panel can display and control up to two Bartender4 action bars per row."] = "底部面板每行最多可以显示和控制两个 Bartender4 操作栏。";
L["Side Bartender4 Action Bars"] = "Side Bartender4 动作条";
L["Set Column Spacing"] = "设置列间距";
L["Column"] = "柱子";
L["Power"] = "玩家力量";
L["Positioning and Visibility Options"] = "定位和可见性选项";
L["Configure each type of anchor point and tooltip type"] = "配置每种类型的锚点和工具提示类型";
L["Assigning tooltips to the mouse anchor point will fix them to your mouse cursor, causing them to follow your mouse movements."] = "将工具提示分配给鼠标锚点会将它们固定到您的鼠标光标，使它们跟随您的鼠标移动。";
L["Assigning tooltips to the screen anchor point will fix them to that single spot and will not move."] = "将工具提示分配给屏幕锚点会将它们固定到该单个点并且不会移动。";
L["World units are 3D player models within the world and are not part of the UI."] = "世界单位是世界中的 3D 播放器模型，不是 UI 的一部分。";
L["Unit frame tooltips are the UI frames that represent NPCs or players."] = "单位框架工具提示是代表 NPC 或玩家的 UI 框架。";
L["Standard tooltips are any other tooltip with no special category (i.e., inventory item and spell tooltips have their own unique category and will not be affected)."] = "标准工具提示是任何其他没有特殊类别的工具提示（即库存物品和法术工具提示有自己独特的类别，不会受到影响）。";
L["Unit Frame Tooltips"] = "单位框架工具提示";
L["World Unit Tooltips"] = "世界单位工具提示";
L["Standard Tooltips"] = "标准工具提示";
L["Choose an Anchor Point:"] = "选择锚点：";
L["Visibility Options:"] = "可见性选项：";
L["Set Shown"] = "设置显示";
L["If unchecked, tooltips of this type will never show"] = "如果未选中，则永远不会显示此类工具提示";
L["If unchecked, tooltips of this type will not show while you are in combat."] = "如果未选中，则在战斗中不会显示此类工具提示。";
L["Hide in Combat"] = "隐藏在战斗中";
L["Mouse"] = "老鼠";
L["Screen"] = "屏幕";
L["Mouse Anchor Point"] = "鼠标锚点";
L["Screen Anchor Point"] = "屏幕锚点";
L["Export Profile"] = "导出配置文件";
L["Export the current profile into a string that can be imported by other players."] = "将当前配置文件导出为其他玩家可以导入的字符串。";
L["Copy the import string below and give it to other players so they can import your current profile."] = "复制下面的导入字符串并将其提供给其他玩家，以便他们可以导入您当前的个人资料。";
L["Close"] = "关闭";
L["Import Profile"] = "导入配置文件";
L["Import a profile from another player from an import string."] = "从导入字符串中导入另一个玩家的配置文件。";
L["Paste an import string into the box below to import a profile."] = "将导入字符串粘贴到下面的框中以导入配置文件。";
L["Warning: This will completely replace your current profile with the imported profile settings!"] = "警告：这将用导入的配置文件设置完全替换您当前的配置文件！";
L["Successfully imported profile settings into your current profile!"] = "已成功将配置文件设置导入您当前的配置文件！";
L["Reset to default"] = "重置为默认";
L["Appearance Settings"] = "外观设置";
L["MUI_FRAMES_COLOR_TOOLTIP"] = "控制 MUI 框架的背景颜色，包括库存框架、工具提示、配置菜单、布局工具等。";
L["Set Display Mode"] = "设置显示模式";
L["These settings relate to the individual aura icons/bars."] = "这些设置与各个光环图标/栏相关。";
L["These settings relate to the frame containing the individual aura icons/bars."] = "这些设置与包含各个光环图标/栏的框架相关。";
L["Set Non-Player Alpha"] = "设置非玩家 Alpha";
L["Icon Width"] = "图标宽度";
L["Icon Height"] = "图标高度";
L["If true, when the aura is close to expiring the aura frame will fade in and out."] = "如果为 true，当光环接近到期时，光环框架将淡入淡出。";
L["The alpha of auras not applied by you."] = "您未应用的光环的阿尔法。";
L["Icon Spacing"] = "图标间距";
L["Show Bar Spark"] = "显示栏火花";
L["Container Frame Settings"] = "容器框架设置";
L["Positioning"] = "定位";
L["Rows and Columns"] = "行和列";
L["The maximum number of aura icons to display per row."] = "每行显示的光环图标的最大数量。";
L["The maximum number of horizontal bar columns to display."] = "要显示的水平条形列的最大数量。";
L["Text Settings"] = "文本设置";
L["Aura Frame"] = "光环框";
L["Icon Frame"] = "图标框";
L["Icon"] = "图标";
L["Normal Font Size"] = "正常字号";
L["Warning Font Size"] = "警告字体大小";
L["Warning Threshold"] = "警告阈值";
L["The minimum number of seconds remaining required for the time remaining text to use the warning font size."] = "剩余时间文本使用警告字体大小所需的最小剩余秒数。";
L["Text Colors"] = "文字颜色";
L["Aura Type Colors"] = "光环类型颜色";
L["Basic Buff"] = "基本增益";
L["Player Owned Buff"] = "玩家拥有的增益";
L["Buffs that you applied to yourself."] = "您为自己应用的增益效果。";
L["Basic Debuff"] = "基本减益";
L["Bar Colors"] = "条形颜色";
L["Borders"] = "边框";
L["Enable Inventory Frame"] = "启用库存框架";
L["Use the MayronUI custom inventory frame instead of the default Blizzard bags UI."] = "使用 MayronUI 自定义库存框架而不是默认的暴雪包 UI。";
L["Use Class Colors"] = "使用类颜色";
L["If checked, tooltips for other players will be colored based on their class."] = "如果选中，其他玩家的工具提示将根据他们的职业着色。";
L["Switch to Detailed View"] = "切换到详细视图";
L["Switch to Grid View"] = "切换到网格视图";
L["View Character Inventory"] = "查看角色清单";
L["Toggle Bags Bar"] = "切换包袋栏";
L["Quest Items"] = "任务物品";
L["Trade Goods"] = "贸易商品";
L["Consumables"] = "耗材";
L["Equipment"] = "设备";
L["All Items"] = "所有项目";
L["Padding"] = "填充";
L["Slot Height"] = "插槽高度";
L["Max Slot Width"] = "最大槽宽";
L["Min Slot Width"] = "最小槽宽";
L["If checked, item levels will show on top of the icons of equipment and weapon items."] = "如果选中，物品等级将显示在装备和武器物品图标的顶部。";
L["The maximum number of icons that can appear per row when resizing the inventory frame using the grid view."] = "使用网格视图调整库存框架大小时每行可以显示的最大图标数。";
L["Max Icons Per Row"] = "每行最大图标数";
L["Show Quest Items Tab"] = "显示任务物品标签";
L["Show Trade Goods Tab"] = "显示贸易商品标签";
L["Show Consumables Tab"] = "显示耗材选项卡";
L["Show Equipment Tab"] = "显示设备选项卡";
L["Show Tab Buttons"] = "显示选项卡按钮";
L["Tab Button Options"] = "选项卡按钮选项";
L["MUI Frames Color"] = "MUI 框架颜色";
L["Theme Color"] = "主题色";
L["Class Color"] = "班级颜色";
L["Color Scheme"] = "配色方案";
L["Font Options"] = "字体选项";
L["Show Item Name"] = "显示项目名称";
L["Item Name Font Size"] = "项目名称字体大小";
L["If checked, item levels will be shown in description of equipment and weapon items."] = "如果选中，物品等级将显示在设备和武器物品的描述中。";
L["Show Item Type"] = "显示项目类型";
L["Item Description Font Size"] = "项目 描述 字体大小";
L["Grid View"] = "网格视图";
L["Detailed View"] = "详细视图";
L["Settings"] = "设置";
L["Points of Interest"] = "兴趣点";
L["If checked, the points of interest arrows will be shown."] = "如果选中，将显示兴趣点箭头。";
L["You have new messages!"] = "您有新消息！";
L["Clear Chat Messages"] = "清除聊天消息";