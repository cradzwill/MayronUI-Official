...
function Move(src, dst)
    ...
    if (C_Container) then
        ...
        canMove = srcContainerInfo and not srcContainerInfo.isLocked and (not dstContainerInfo or not dstContainerInfo.isLocked);
    else
        ...
        canMove = texture and not srcLocked and not dstLocked;
    end
	...
	function Initialize()
		...
		for _, container in ipairs(CONTAINERS) do
			...
			for position = 1, GetContainerNumSlots(container) do
				...
				if item then
                    local isLocked, total;

                    if (C_Container) then
                        local containerInfo = C_Container.GetContainerItemInfo(container, position);
                        isLocked = containerInfo and containerInfo.isLocked;
                        total = containerInfo.stackCount
                    else
					...
					slot.item = item
					slot.count = total
					counts[item] = (counts[item] or 0) + total
				end
				insert(model, slot)
			end
		end
