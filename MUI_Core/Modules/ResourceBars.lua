...
C_ResourceBarsModule.Static:AddFriendClass("MainContainer");
...
local function ApplyVisualUpdates(_, _, container, bars)
  ...
  ---@type BottomActionBarsModule
  local actionBarsModule = MayronUI:ImportModule("BottomActionBars");

  if (actionBarsModule and actionBarsModule:IsEnabled()) then
    actionBarsModule:SetUpExpandRetract();
  end
end
...
function C_ResourceBarsModule:OnEnable(data)
  ...
  data.barsContainer = tk:CreateFrame("Frame", _G["MUI_BottomContainer"], "MUI_ResourceBars");
...
function C_ResourceBarsModule:SetBlockerEnabled(data, enabled, dataTextBar)
  if (not data.blocker and enabled) then
    data.blocker = tk:CreateFrame("Frame", data.barsContainer);
...
function C_BaseResourceBar:CreateBar(data)
  ...
  local statusbar = tk:CreateFrame("StatusBar", frame);
